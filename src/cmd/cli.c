...
static void set_input_file(CmdArgs* args, const char* value) {
    if (!value) handle_error(ERR_INVALID_FILE_LOCATION, NULL, NULL, INPUT_FILE_ARGUMENT);
...
static void set_output_file(CmdArgs* args, const char* value) {
    if (!value) handle_error(ERR_INVALID_FILE_LOCATION, NULL, NULL, OUTPUT_FILE_ARGUMENT);
