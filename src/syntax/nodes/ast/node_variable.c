...
void parse_variable(Tokenizer* tokenizer, ASTNode* ast_node) {
    ...
    if (peek(tokenizer, SYMBOL_SEMICOLON)) {
        consume(tokenizer);
        ASTNode* variable_content_node =
            create_ast_node(AST_IDENTIFIER_VARIABLE_CONTENT, nullptr);
        ds_astnode_ptr_array_push(variable_node->nodes, variable_content_node);
        ds_astnode_ptr_array_push(ast_node->nodes, variable_node);
        return;
    }
    ...
    ds_astnode_ptr_array_push(variable_content_node->nodes, expression);
    ds_astnode_ptr_array_push(variable_node->nodes, variable_content_node);
    ds_astnode_ptr_array_push(ast_node->nodes, variable_node);
...
ASTNode* parse_variable_declaration(Tokenizer* tokenizer) {
    ...
    ASTNode* variable_type_node =
        create_ast_node(AST_IDENTIFIER_VARIABLE_TYPE, type_token);
    ASTNode* variable_node =
        create_ast_node(AST_IDENTIFIER_VARIABLE_DEFINITION, name_token);

    ds_astnode_ptr_array_push(variable_node->nodes, variable_type_node);
...
void resolve_variable_definition(ASTNode* node, SymbolTable* symbol_table,
    ...
    ASTNode* variable_type_node = ds_astnode_ptr_array_get(node->nodes, 0);
